// === GENERIERT von scripts/migrate-refs-v25.ts — nicht von Hand editieren ===
// Plan: docs/superpowers/plans/2026-06-01-loesungsfinder-4step-adaptive.md
//
// flaecheKategorie + zeitfenster: deterministisch aus V2.4-Daten.
// einsatzbereich + innenAussen: aus einsatzbereiche[0] abgeleitet (Review empfohlen).
// schadenstypen: DRAFT (Keyword-Extraktion) — Sign-off Frank offen.
// Neu generieren: npx tsx scripts/migrate-refs-v25.ts

import type { ReferenzFilterV25 } from "./types";

export const REFERENZ_FILTER_V25: Record<string, ReferenzFilterV25> = {
  "antolin-wochenend-sanierung": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "kleemann-produktionshalle": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden", "risse"],
  },
  "monheim-produktionsflaeche": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "weag-entsorgungsbetrieb": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "wochenend-sanierung-werkstatt": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden", "risse"],
  },
  "strandkorbhalle-sylt": {
    flaecheKategorie: "gross",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden"],
  },
  "loosen-werkzeug-klausen": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["risse"],
  },
  "guben-produktionshalle": {
    flaecheKategorie: "gross",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "nike-store-polen": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-verkauf-showroom",
    zeitfenster: "planbar",
    schadenstypen: [],
  },
  "obstplantage-ibbenbueren": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "sanierung-einer-sanierung": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "dhl-ueberadebruecken": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "lkw-waschstrasse": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden"],
  },
  "helipad-sanierung-polen": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden", "ausbrueche", "risse"],
  },
  "fugensanierung-lyreco": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "treppenstufen-sanierung": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "sinusfugen-sanierung": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "trennfugen-bohnenkamp": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "absenksteine-tankstelle": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "lkw-einstellplatz-berlin": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "hafen-catania": {
    flaecheKategorie: "gross",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: [],
  },
  "parkhaus-flughafen-zuerich": {
    flaecheKategorie: "gross",
    innenAussen: "aussen",
    einsatzbereich: "aussen-parkdeck-tiefgarage",
    zeitfenster: "planbar",
    schadenstypen: [],
  },
  "theodor-heuss-bruecke": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden"],
  },
  "autohaus-versmold": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden"],
  },
  "hubschrauber-landeplatz-finnland": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "planbar",
    schadenstypen: ["verschleissschaeden"],
  },
  "lkw-umfahrt-darmstadt": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "sanierung-lkw-zufahrt-logistikzentrum-sankt": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "texaco-tankflache-arnheim": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "naturex-burgdorf": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lebensmittel-pharma",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "torschwelle-lagerhalle": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "wellpappenfabrik-gross-versmold": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "korodur-demo-bochum": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "kreisverkehr-goppingen": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "barmenia-parkhaus-wuppertal": {
    flaecheKategorie: "mittel",
    innenAussen: "aussen",
    einsatzbereich: "aussen-parkdeck-tiefgarage",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "john-lewis-lager-stevenage": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "fraport-frankfurt": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "sncf-bordeaux": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "parkhaus-freiburger-munster-freiburg": {
    flaecheKategorie: "gross",
    innenAussen: "innen",
    einsatzbereich: "innen-lebensmittel-pharma",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden", "frueher-sanierung"],
  },
  "olympiastadion-berlin": {
    flaecheKategorie: "gross",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "wirtgen-produktionshallen-weltweit": {
    flaecheKategorie: "gross",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "fh-lichtschacht-nurnberg": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "klaranlage-nako": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-verkauf-showroom",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "bruckensanierung-amberg": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "parkplatzsanierung-metzingen": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-parkdeck-tiefgarage",
    zeitfenster: "sehr-kurz",
    schadenstypen: ["verschleissschaeden", "ausbrueche", "risse"],
  },
  "schachtregulierung-fahrbahn-nittenau": {
    flaecheKategorie: "punktuell",
    innenAussen: "innen",
    einsatzbereich: "innen-lager-logistik",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "treppensanierung-gehweg-esslingen": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "burger-king-munster": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-lebensmittel-pharma",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "kraftwerk-bergamo": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-industrie-produktion",
    zeitfenster: "kurz",
    schadenstypen: ["verschleissschaeden"],
  },
  "fahrbahnsanierung-wien": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-infrastruktur-verkehr",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "oelie-saur-saint": {
    flaecheKategorie: "punktuell",
    innenAussen: "aussen",
    einsatzbereich: "aussen-parkdeck-tiefgarage",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
  "decathlon-dortmund": {
    flaecheKategorie: "mittel",
    innenAussen: "innen",
    einsatzbereich: "innen-verkauf-showroom",
    zeitfenster: "sehr-kurz",
    schadenstypen: [],
  },
};
